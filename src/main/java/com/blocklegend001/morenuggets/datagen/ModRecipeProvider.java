...
public class ModRecipeProvider extends FabricRecipeProvider {
    ...
    public void generate(RecipeExporter exporter) {
        MATERIAL_TO_NUGGET.forEach((material, nugget) -> addNuggetRecipes(material, nugget.get(), exporter));
        addSmeltingRecipes(exporter);
    }

    private void addNuggetRecipes(Item material, Item nugget, RecipeExporter recipeOutput) {
        ...
        ShapelessRecipeJsonBuilder.create(RecipeCategory.MISC, nugget, 9)
                ...
                .offerTo(recipeOutput, Identifier.of(MoreNuggets.MOD_ID, nuggetId.getPath() + "_from_" + materialId.getPath()));

        ShapedRecipeJsonBuilder.create(RecipeCategory.MISC, material)
                .pattern("NNN").pattern("NNN").pattern("NNN")
                .input('N', nugget)
                .criterion("has_" + nuggetId.getPath(), InventoryChangedCriterion.Conditions.items(nugget))
                .offerTo(recipeOutput, Identifier.of(MoreNuggets.MOD_ID, materialId.getPath() + "_from_" + nuggetId.getPath()));
    }
    ...
    private void addSmeltingRecipes(RecipeExporter recipeOutput) {
        ...
        nuggetToItems.forEach((nugget, items) -> items.forEach(item -> {
            ...
            CookingRecipeJsonBuilder.create(Ingredient.ofItems(item), RecipeCategory.MISC, nugget, 1.0f, 100, RecipeSerializer.SMELTING, SmeltingRecipe::new)
                    ...
                    .offerTo(recipeOutput, Identifier.of(MoreNuggets.MOD_ID, itemId.getPath() + "_to_" + nuggetId.getPath()));
        }));
    }
}
