...
public class ModRecipeProvider extends RecipeProvider {
    ...
    protected void buildRecipes() {
        MATERIAL_TO_NUGGET.forEach((material, nuggetSupplier) -> addNuggetRecipes(material, nuggetSupplier.get(), this.output));

        addSmeltingRecipes(this.output);
    }
    ...
    private void addNuggetRecipes(Item material, Item nugget, RecipeOutput recipeOutput) {
        shapeless(RecipeCategory.MISC, nugget, 9)
                ...
                .save(this.output, String.valueOf(ResourceLocation.fromNamespaceAndPath(MoreNuggets.MOD_ID, Objects.requireNonNull(ForgeRegistries.ITEMS.getKey(nugget)).getPath() + "_from_" + Objects.requireNonNull(ForgeRegistries.ITEMS.getKey(material)).getPath())));

        shaped(RecipeCategory.MISC, material)
                ...
                .save(this.output, String.valueOf(ResourceLocation.fromNamespaceAndPath(MoreNuggets.MOD_ID, Objects.requireNonNull(ForgeRegistries.ITEMS.getKey(material)).getPath() + "_from_" + Objects.requireNonNull(ForgeRegistries.ITEMS.getKey(nugget)).getPath())));
    }
    ...
    private void addSmeltingRecipes(RecipeOutput recipeOutput) {
        ...
        nuggetToItems.forEach((nugget, items) -> items.forEach(item -> SimpleCookingRecipeBuilder.smelting(Ingredient.of(item), RecipeCategory.MISC, nugget, 1.0f, 100)
                ...
                .save(this.output, String.valueOf(ResourceLocation.fromNamespaceAndPath(MoreNuggets.MOD_ID, Objects.requireNonNull(ForgeRegistries.ITEMS.getKey(item)).getPath() + "_to_" + Objects.requireNonNull(ForgeRegistries.ITEMS.getKey(nugget)).getPath())))));
    }
}